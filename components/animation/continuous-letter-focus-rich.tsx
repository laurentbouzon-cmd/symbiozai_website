...
interface ContinuousLetterFocusRichProps {
  children: string
...
export function ContinuousLetterFocusRich({
  children,
  tag = "p",
  className = "",
...
}: ContinuousLetterFocusRichProps) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (containerRef.current) {
        ...
        // Traiter le texte pour préserver les sauts de ligne
  ...
  return (
    <div ref={containerRef} className={`continuous-letter-focus-rich ${className}`}>
      {/* Le contenu sera généré dynamiquement via useEffect */}
    </div>
  )
}
